...
from .block import EMA, C3, C2f, C3k
 
__all__ = ("C2f_FasterBlock", "C2f_FasterBlock_EMA", "C3_Faster_CGLU", "C2f_Faster_CGLU", "C3k2_FasterBlock")
...
class FasterBlock_EMA(nn.Module):
    def __init__(self,
                 inc,
                 dim,
                 n_div=4,
...
class C2f_FasterBlock_EMA(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(FasterBlock_EMA(self.c, self.c) for _ in range(n))
...
class C3k2_FasterBlock(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else FasterBlock(self.c, self.c) for _ in range(n)
        )
        # 解析 c3k在主干和网络最后一个C3k2的时候设置True走C3k, 否则MSBlock
