...
def _bbox_iou_kwargs(hyp) -> dict:
    """
    Map the `iou_type` / `Focaleriou` / `d` / `u` hyperparameters (see default.yaml's "IoU Settings" block)
    onto the keyword arguments `bbox_iou` expects.

    Falls back to plain CIoU (Ultralytics' original default) when `hyp` is None or does not define an
    `iou_type`, so configs that predate this option keep training exactly as before.
    """
    if hyp is None:
        return {"CIoU": True}
    iou_type = str(getattr(hyp, "iou_type", "CIoU")).upper()
    kwargs = {
        "GIoU": iou_type == "GIOU",
        "DIoU": iou_type == "DIOU",
        "CIoU": iou_type == "CIOU",
    ...
    if not any(kwargs.values()):
        kwargs["CIoU"] = True  # unrecognized/legacy iou_type -> fall back to CIoU rather than plain IoU
    if getattr(hyp, "Focaleriou", False):
        kwargs["Focaler"] = True
        kwargs["focaler_d"] = getattr(hyp, "d", 0.0)
        kwargs["focaler_u"] = getattr(hyp, "u", 0.95)
    return kwargs
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, hyp: Any = None):
        """Initialize the BboxLoss module with regularization maximum, DFL settings, and IoU hyperparameters."""
        super().__init__()
    ...
    def forward(
        ...
        iou = bbox_iou(
            pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, **_bbox_iou_kwargs(self.hyp)
        )
